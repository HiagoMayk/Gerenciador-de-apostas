...
public abstract class RegraJogo implements IRegraJogo
{
	...
	public boolean aplicarRegraJogo(Aposta aposta, IDAOAposta apostas, IDAOApostador apostadores)
	{
		...
		if((verificaQuantidadeAposta(apostas) && verificaFaixaValores(aposta) &&
			verificaDominioAposta(aposta) && verificarManeiraAposta(aposta, apostadores)) == true)
		{
	...
	public boolean verificaDominioAposta(Aposta aposta)
	{
            ...
            {
                for(int j = 0; j < io.getObjetosAposta().size(); j++) // dominio da aposta
                {
